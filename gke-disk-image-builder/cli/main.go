...
func main() {
	...
	diskSizeGb := flag.Int64("disk-size-gb", 100, "disk size for building/unpacking images")
	outputDiskSizeGb := flag.Int64("output-disk-size-gb", 0, "output disk image size (defaults to disk-size-gb if not specified)")
	...
	// Default output-disk-size-gb to disk-size-gb if not specified (backward compatibility)
	if *outputDiskSizeGb == 0 {
		outputDiskSizeGb = diskSizeGb
	}
	...
	req := builder.Request{
		...
		OutputDiskSizeGB:      *outputDiskSizeGb,
		GCPOAuth:              *gcpOAuth,
